...
export class BusinessPrivateService {
  ...
  updateBusinessEmail(data: UpdateBusinessEmailInput): Observable<BusinessSchema> {
    return this.apollo
      .use('businessAPI')
      .mutate<{ updateBusinessEmail: BusinessSchema }>({
        mutation: UPDATE_BUSINESS_EMAIL_MUTATION,
        variables: { data },
        context: {
          withCredentials: true,
        },
